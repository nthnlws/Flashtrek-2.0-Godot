...
static func generate_neutral_ship_data(spawn_position: Vector2, faction: Utility.FACTION, difficulty: float, index: int) -> NeutralShipData:
	var new_neutral_data = NeutralShipData.new()
	
	new_neutral_data.ship_index = index
	new_neutral_data.world_position = spawn_position
